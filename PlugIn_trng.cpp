...
int cbFlipEffectMine(WORD FlipIndex, WORD Timer, WORD Extra, WORD ActivationMode)
{
	...
	switch (FlipIndex) {
		...
		// 100: Puzzle. Moving Plaques. Initialize puzzle for current level.
	case 100:
...
void Mehrbod::TLSFL::TLSFL_Functions::Parameter_PlayLoopedSFX_Animating()
{
	/*Get(enumGET.GAME_INFO, NULL, NULL);

	if (GET.GameInfo.LevelIndex == 1 && Get(enumGET.ITEM, 1009 | NGLE_INDEX, NULL))
	{
		if (!displayedMsg)
		{
			displayedMsg = true;
		}
		GET.pItem->OrientationT = GET.pItem->OrientationT + 300;
		StrTriplePoint point;
		point.CordX = GET.pItem->CordX; point.CordY = GET.pItem->CordY; point.CordZ = GET.pItem->CordZ;
		SoundEffect(40, &point, NULL);
	}
}*/
...
void Mehrbod::TLSFL::Functions::FlipEffect_Puzzles_MovingPlaques_Initialize()
{
	...
	for (int i = 0; i < numPlaques; i++)
	{
		...
		{
		case 0:
			plauqeNow->ClockwiseCogs[0] = 1010;
